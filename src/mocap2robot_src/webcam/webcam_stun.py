...
def create_local_tracks(play_from, decode):
    ...
    if play_from:
        ...
        # options = {"framerate": "30", "video_size": "2560x720"}
        if relay is None:
            if platform.system() == "Darwin":
                ...
                "video=ZED-M", format = "dshow", options = options
                )
            else:
...
async def offer(request):
    ...
    iceServers=[
        ...
        RTCIceServer(
            urls=['turn:192.168.112.214:3478'],  # TURN 服务器
            username='username1',  # TURN 用户名
            credential='key1'  # TURN 密码
        )
    ...
    async def on_icecandidate(candidate):
        if candidate is None:
            ...
            # 打印不同类型的候选地址
            print(f"ICE Candidate: {candidate.component}")
            print(f"Type: {candidate.type}")  # host, srflx, relay
